...
program estimate_dist_lag_model 
    syntax [if], depvar(str) dyn_var(str) stat_var(str)          ///
        ...
        [wgt(str) ab test_equality outfolder(str) w(int 6) save_res_zip_month]
    ...
    local resid_opt ""
    if "`save_res_zip_month'" != "" {
        local resid_opt "resid"
    }
    ...
    if "`ab'"=="" {
        ...
        if inlist("`dyn_var'", " ", "") {
            reghdfe D.`depvar' `contlist' `wgtsyntax' `if',         ///
            ...
            reghdfe D.`depvar' L(-`w'/`w').D.`dyn_var' D.`stat_var' ///
                    ...
                    vce(cluster `cluster') nocons `resid_opt' 
        }             
    }
    else {
        ivreghdfe D.`depvar' L(-`w'/`w').D.`dyn_var' D.`stat_var'         ///
                (L.D.`depvar'=L2.D.`depvar') `contlist' `wgtsyntax' `if', ///
                absorb(`absorb') cluster(`cluster') nocons `resid_opt'
    }
