...
build_basic_stats <- function(df) {
  df %>%
    summarise(tot_pop_cens2010              = sum(population_cens2010, na.rm = T)/1000,
              ...
              share_wage_hhlds_irs2014      = mean(share_wage_hhlds, na.rm = T),
              share_bussiness_hhlds_irs2014 = mean(share_bussiness_hhlds, na.rm = T),
              agi_per_hhld_irs_2014         = mean(agi_per_hhld, na.rm = T)/1000,
              wage_per_hhld_irs2014         = mean(wage_per_hhld, na.rm = T)/1000,
              rent40thperc_2br_safmr2014    = mean(safmr2br, na.rm = T),
              min_binding_mw_dec2014        = min(statutory_mw_dec2014, na.rm = T),
              avg_binding_mw_dec2014        = mean(statutory_mw_dec2014, na.rm = T),
              max_binding_mw_dec2014        = max(statutory_mw_dec2014, na.rm = T),
