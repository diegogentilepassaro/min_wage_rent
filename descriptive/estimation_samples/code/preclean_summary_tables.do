...
program main
    ...
    save_data "../temp/acs_2011_clean.dta",  log(none)      ///
        key(zcta) replace
    ...
    save_data "../temp/safmr_clean.dta",     log(none)      ///
        key(zipcode countyfips cbsa10 year) replace
    
    keep if year == 2012
    drop year
    save_data "../temp/safmr_2012_clean.dta", log(none)     ///
        key(zipcode countyfips cbsa10)  replace
    
    build_zip_lvl_samples, instub(`instub_est_samp')

    foreach data in "all"              "all_urban"          ///
                    "all_zillow_rents" "baseline_zillow_rents" {
