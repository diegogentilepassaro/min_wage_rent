...
\begin{abstract}
    ...
    For each location we define 
    both the minimum wage where the average resident works (the "workplace MW") 
    and the minimum wage in the location itself (the "residence MW").
    We write down a partial-equilibrium model in which minimum wage levels in
    each location affect housing demand by changing the income of commuters and 
    the prices of 
    non-tradable consumption. 
    We show that, in the model, the workplace MW increases rents whereas the 
    residence MW has a negative effect on rents.
