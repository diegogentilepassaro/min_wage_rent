...
$h^{\pre}_i = h^{\post}_i = h_i$ so that
...
\[
    \Delta \ln R_i = \beta \MW_i^{\wkp} + \gamma \MW_i^{\res}
\]

Now, we also have to define the change in log income as

\[
    \Delta \ln Y_i = \varepsilon \MW_i^{\wkp}
\]
...
\begin{equation}\label{eq:wage_level_model}
    \ln Y_{iy} = \alpha_i + \alpha_y + \varepsilon \overline{\MW_{iy}^{\wkp}} + 
...
\begin{equation}\label{eq:rho}
	\begin{split}
		...
		\right] \\
		& = s_i \left[
		\frac{\exp \left( \beta \MW_i^{\wkp} + \gamma \MW_i^{\res} \right) - 1 }{\exp \left( \varepsilon \MW_i^{\wkp} \right) - 1 }
		\right]
	\end{split}
\end{equation}
