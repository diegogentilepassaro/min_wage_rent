...
application.
For instance, if there were a single national minimum wage level or everybody 
resides and works in the same location 
(i.e., for all ZIP codes we have $\pi_{ii}=1$), then we would have
...
\begin{equation*}\label{eq:strict_exogeneity}
    E\left[
        ...
        \end{pmatrix}
        \Delta \varepsilon_{it}
    \bigg| \delta_t, \Delta \mathbf{X}_{c(i)t} \right] =
    \begin{pmatrix}
        0 \\
        0 \\
    \end{pmatrix}
...
feedback effects in one specification described in the upcoming subsection.
...
Under the assumption that there is no anticipatory effects in the housing market, we 
...
systematically affect both rents and minimum wage changes. We deploy two strategies to account for that. First 
