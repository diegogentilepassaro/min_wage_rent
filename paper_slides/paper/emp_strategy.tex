...
We assume that these sectors are not affected by the minimum wage.\footnote{These sectors 
	have a very small share of MW workers (ADD REFERENCE). In appendix figures XXX, we show 
...
shocks driving our results. We think that, given the high frequency of our data and that we focus 
on short windows around MW changes, the assumption of fixed supply is reasonable. We further 
present evidence in favor of this assumption by showing that MW changes do not predict the number 
of listings of houses for sale in Zillow.\footnote{Ideally, we would run this regression on the 
	number of rental units. Unfortunately, as described in \autoref{sec:data}, this information 
	is not available.}
...
Identification follows from strict exogeneity as well.

This model allows us to estimate the dynamics of the logarithm of the rent per square foot 
around changes in the MW. Now, the cumulative elasticity of rents to MW is defined as $\sum_{r=0}^s 
...
effects of MW changes based on zipcode characteristics, therefore assessing whether our effects 
are driven by zipcodes expected to have more MW workers. Formally, the model is
...
the actual MW (i.e., zipcodes that experience a negative shock). We discuss results using the 
