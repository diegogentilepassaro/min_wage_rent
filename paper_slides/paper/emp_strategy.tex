...
In Appendix \ref{sec:assigning_mw_levels} we explain our estimation of the 
share of MW workers in the US ZIP codes. 

We then estimate the following model:
\begin{equation}\label{eq:fd_heterogeneity}
    \Delta r_{it} = \delta_t
                  + \gamma \Delta \mw^{\res}_{it} + \gamma^h I_i \Delta \mw^{\res}_{it}
                  + \beta \Delta \mw^{\wkp}_{it} \beta^h \Delta I_i \mw^{\wkp}_{it}
                  + \Delta \mathbf{X}^{'}_{it}\eta
                  + \Delta \varepsilon_{it} ,
\end{equation}
where $I_i$ represents the standardize share of MW workers in a ZIP code.
We conduct a similar heterogeneity exercises with the standardized measure of
median household income \parencite[ACS;][]{CensusACS}, and with indicator for whether the ZIP
