...
factors or zipcode-specific trends, preexisting time paths of rents per square foot relative to the MW changes may  be different in treated and non-treated zipcodes due uncontrolled demand shocks or anticipatory effects 
...
shocks driving our results. We think that, given the high frequency of our data and that we focus on short windows around MW changes, the assumption of fixed supply is reasonable. We further present evidence in favor of this assumption by showing that MW changes do not predict the number of listings of houses for sale in Zillow.\footnote{Ideally, we would run this regression on the number of rental units. Unfortunately, as described in \autoref{sec:data}, this information is not available.}
