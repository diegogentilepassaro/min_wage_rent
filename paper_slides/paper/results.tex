...
MW on housing rents. \textcite{Yamagishi2019} uses county-year variation,
...
month panel in which we assigned MW measures applying the analogous procedure described in
\ref{sec:blocks_to_uspszip} and we used Zillow data available at that unit. The results in 
row (iii) are similar in magnitude to our baseline results but they are not significant and 
much less precisely estimated. In figure XXX, we show that at the county month there are 
considerable pretrends in the rental prices.
%%% DGP: I think that we should have a dynamic baseline figure only with the leads and lags of 
%%% the workplace MW and compare here with that figure the same of the county-month. Then
%%% we can place the otehr two subfigure currently in dynamic baseline in an appendix. 
...
well suited for estimating the effects of the MW changes. 

%%% DGP: Add comments comparing with Yamagishi and Tidemann using rows (i) of the different models. 
