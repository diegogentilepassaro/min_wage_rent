...
Next, we compare our estimates to existing research on the effect of 
the MW on rents.
Exploiting heterogeneity in MW levels across Japanese prefectures,
\textcite{Yamagishi2021} estimates an elasticity of 0.25--0.45. Those estimates
are an order of magnitude larger than to our estimated impact of an increase 
in both the residence and workplace MW. Even if they also use posted rents, and that 
we recognize that the rental market may be substantially different in Japan that in 
the US, in light of the literature of the effect of the MW on income and on consumption 
prices, we see our estimated magnitudes as more plausible. To illustrate that with an 
example, suppose that in a certain geography the MW increases 10\%. Using an elasticity 
of household income to the MW of 0.3 (a large elasticity 
for an entire geography based on the results in \textcite{Dube2019Income}) we would have that 
household income would increase only 3\%. Therefore, we see rents increasing 2.5\%--4.5\% 
in that geography as an implausibly large effect, specially because not all households are 
renting.
...
Our baseline ZIP code-level estimates yield elasticities in the lower 
end of that range.%
...
an elasticity of 0.73, an order of magnitude larger than our estimates. One reason
for divergence is that in their study nominal rents increase for the individuals 
affected by the MW change and not for an entire ZIP code containing unaffected 
individuals like in our study.While it is hard to pin down the exact reason 
for this divergence given the multiple differences with our setting, we see 
their value as large given the existing literature on income, price, and rent effects.
