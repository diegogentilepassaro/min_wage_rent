...
\begin{proof}
    ...
    $$\left(\eta_i - \sum_z \pi_{iz} \xi^R_{iz}\right)^{-1} 
      \pi_{iz}\xi^Y_{iz}\epsilon_{iz}^Y d\ln\MW_z.$$
    Because $\eta_i>0$ and $\xi^R_{iz} < 0$ for all $z\in\Z(i)$, 
    the first factor is positive.
    Since we also assumed $\epsilon_{iz}^Y>0$ this effect is positive, as 
    desired.

    For ZIP code $i$ the partial effect is given by
    $$\left(\eta_i - \sum_z \pi_{iz} \xi^R_{iz}\right)^{-1} 
      \left(\epsilon_{i}^P \sum \pi_{iz}\xi^P_{iz} 
            + \pi_{ii}\xi^Y_{ii}\epsilon_{ii}^Y \right)d\ln\MW_i.$$
    Because $\epsilon_{i}^P>0$, $\xi^P_{iz}<0$ for all $z\in\Z(i)$,
    and $\epsilon_{i}^Y>0$, 
...
market.
First, unless the heterogeneity in $\{\xi_{iz}^Y\epsilon_{iz}^Y\}_z$ is strongly 
asymmetric, 
...
$\xi^Y_{iz} \epsilon_{iz}^Y$ is homogeneous within each ZIP code but varies
