...
%% 
%% Specifically, normal goods rule out the possibility of rents being Giffen good
%%
...
As apparent from the proof of Proposition 1, the effect of the MW on rents
...
plausible if the share of low-wage workers within each flow $L_{iz}$ does not 
vary by $z$.
In that case all $(i,z)$ groups will react similarly to a MW increase.
...
We note, however, that this assumption holds trivially for all preferences such 
that $h_{iz} = g\left(R_i, P_i\right) Y_i$ for some $g\left(\cdot\right)$, 
such as Cobb-Douglas or Constant Elasticity of Substitution.

We see that these assumptions will hold only in special cases, which suggests
that they are indeed strong assumptions.
However, we maintain that the workplace MW measure provides a useful 
approximation to study the spillover of MW policies on the housing market.
First, because these elasticities are likely to be larger for $(i,z)$ groups 
that have many low-wage workers, we expect to understate the importance of the 
workplace MW for the housing market.
In other words, $\beta_i  d \mw^{\wkp}_i$ will be too small relative to 
the elasticity-weighted changes in workplace MW levels that, 
according to the model, determine rents.
Second, we note that we conduct several exercises that attempt to account
for some of this heterogeneity.
...
Our main takeaways are unchanged across these exercises.
