...
market.
First, unless the heterogeneity in $\{\xi_{iz}^Y\epsilon_{iz}^Y\}_{z\in\Z(i)}$ 
...
$\xi^Y_{iz}\epsilon_{iz}^Y = \overline{\xi^Y\epsilon^Y}_i + \nu_{iz}$ where 
...
%%
Second, if low-wage workers are concentrated into residence-workplace pairs that
are likely to experience increases in the workplace MW, then we expect to
understate their contribution towards changes in rents (as those flows that are
likely highly responsive to the MW will be down-weighted).
In other words, our approach is conservative.
Finally, we conduct several exercises that attempt to account for heterogeneity
as a function of the share of MW workers residing in $i$.
...
in $i$.
In these exercises our results are stronger.
