...
geographies, i.e., census blocks, census tracts.
Furthermore, the minimum wages are defined at the level of states, counties,
and census places.
...
To assign variables defined at the level of 2010 census 
geographies to USPS ZIP codes we exploit an original correspondence 
table between census blocks and USPS ZIP codes.
We construct this mapping as follows:
(1) we collect the GIS map of census blocks from \textcite{cbTiger} and compute their 
centroids;
(2) we assign the census block to a unique USPS ZIP code using the map from 
\textcite{ESRI} based on assigning to each census block the USPS ZIP code
that contains its centroid.
This creates a unique mapping from census blocks to USPS ZIP codes.


To assign minimum wages defined at county, state, and place
to USPS ZIP codes we also exploit an original correspondence table between 
census blocks and places.
We construct this mapping as follows:
...
that contains its centroid.
This creates a unique mapping from census blocks to places.

Once we constructed these mappings, then assigning variables defined at any
2010 Census geographies to USPS ZIP codes is straightforward.
...
weights are the number of housing units in each census block.
