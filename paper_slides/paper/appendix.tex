...
In the proposition below we show that the parameters $\beta$ and $\gamma$ can be 
recovered from data on rents and minimum wage changes under a suitable parallel 
trends assumption.
...
\begin{prop}[Identification]\label{prop:did_id}
    ...
    for some ZIP codes $z\in\Z_0\subset\Z$ for non-empty $\Z_0$,
        $\Delta\mw_{zt}=\Delta\mw$ if $t=\bar{t}$ and
        $\Delta\mw_{zt}=0$         if $t\neq\bar{t}$,
    and for other ZIP codes $z\in\Z\setminus\Z_0$,
        $\Delta\mw_{zt}=0$ for all $t$.
    Assume
    (1) differential exposure to MWs for ZIP codes with high and low exposure,
    (2) parallel trends across ZIP codes treated and untreated directly, and
    (3) parallel trends across ZIP codes with high and low exposure.
...
\begin{proof}
    ...
    Using the previous equation we can compute, for $t\neq \bar{t}$,
    \begin{equation*}
    ...
    \begin{equation*}
        \begin{split}
            E[\Delta r_{it} | i\in\Z_h] - E[\Delta r_{it} | i\in\Z_l] 
            & = \beta \left(E[\sum_{z\in\Z_0}\pi_{iz} | i\in\Z_h]
                           - E[\sum_{z\in\Z_0}\pi_{iz} | i\in\Z_l]\right) \Delta \mw  \\
            & + E[\Delta \varepsilon_{it} | i\in\Z_h] - E[\Delta \varepsilon_{it} | i\in\Z_l] .
        \end{split}
    \end{equation*}
    Under the assumption that $E[\Delta \varepsilon_{it} | i\in\Z_h] - E[\Delta \varepsilon_{it} | i\in\Z_l]=0$
    ...
    For those ZIP codes that treated directly, the expected evolution of rents is:
    \begin{equation*}\label{eq:expected_rents_treated}
        E[\Delta r_{it} | i\in\Z_0] = 
        \begin{cases}
            \delta_t + E[\Delta \varepsilon_it | z\notin\Z_0] 
                                       & \text{ if } t\neq\bar{t} \\
            \gamma\mw_{it} + \beta\sum_{z\in\Z_0}\pi_{iz}\mw_{zt} + \delta_t 
                     + E[\Delta \varepsilon_it | z\notin\Z_0] 
                                       & \text{ if } t=\bar{t} .
        \end{cases}
    \end{equation*}
    ...
    \begin{equation*}
        \begin{split}
            E[\Delta r_{it} | i\in\Z_0] - E[\Delta r_{it} | i\notin\Z_0] 
              & = \gamma \Delta \mw  \\
              & + \beta \left(E[\sum_{z\in\Z_0}\pi_{iz} | i\in\Z_0]
                           - E[\sum_{z\in\Z_0}\pi_{iz} | i\notin\Z_0]\right) \Delta \mw  \\
    ...
    \begin{equation}\label{eq:id_gamma}
        \gamma = \frac{E[\Delta r_{it} | i\in\Z_0] - E[\Delta r_{it} | i\notin\Z_0]}{\Delta\mw}
            - \beta \left(E[\sum_{z\in\Z_0}\pi_{iz} | i\in\Z_0] - E[\sum_{z\in\Z_0}\pi_{iz} | i\notin\Z_0]\right) .
    \end{equation}
