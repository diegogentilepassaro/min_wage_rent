...
First, we collected the GIS map of census blocks from \textcite{cbTiger} and 
compute their centroids.
Second, we assigned the block to a unique USPS ZIP code using the GIS map 
...
We assign XXXXXXX census blocks using this spatial match 
(XX.XX percent of the total).
%%
%% SH: Fill the XXXXX
%% 
Third, for the blocks that remain unassigned we use the 
tract-to-USPS-ZIP-code correspondece from \textcite{hudCrosswalks}.
...
identifier.
We assign XXXXXXX blocks using this approach (XX.XX percent).
There remain XXXXXX unassigned blocks (XX.XX percent), which 
we drop from the analysis.
%%
%% SH: Fill the XXXXX
%% 
This creates a unique mapping from census blocks to USPS ZIP codes.

There are XXXXXX census blocks which are assigned to XXXXX USPS ZIP codes,
implying an average of XXX census blocks per ZIP code.
%%
%% SH: Fill the XXXXX
%% 
