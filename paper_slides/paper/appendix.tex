...
One challenge of this project is that LODES data on commuting patterns and are 
aggregated at the census blocks level of the 2010 US Census.\footnote{ACS data on 
	socio-demographic characteristics come aggregated at the census block, and at 
	the census block group level when income bins are included.}
However, our main data on rents is aggregated at the level of USPS ZIP codes,
and minimum wage policies are defined at the federal, state, county, and city level.
Fortunately, many different levels of the 2010 census geographies are nested, and the LODES 
data already comes with state, county, place, ZCTA, tract, and block group identifiers for 
each census block. Nevertheless, census blocks may cross USPS ZIP code borders. Therefore,
to assign each census block to a single USPS ZIP cods we do the following:
(1) we collect the GIS map of census blocks from \textcite{cbTiger} and 
compute their centroids
(2) we assign the census block to a unique USPS ZIP code using the GIS map 
from \textcite{ESRI} based on assigning to each census block the USPS ZIP 
code that contains its centroid. If the centroid falls outside of the census 
block, we pick a point inside it at random;
This creates a unique mapping from census blocks to USPS ZIP codes.

Once we constructed these mapping, assigning variables defined at 
any 2010 Census geography to USPS ZIP codes is straightforward.
We start by assigning the given variable to census blocks or census block groups.
Then, we aggregate the variable to the USPS ZIP code level.
For count variables, such as the number of workers in LODES data, we sum them.
...
There are 11,053,116 census blocks which are assigned to 31,755 USPS ZIP codes,
