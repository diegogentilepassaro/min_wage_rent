...
and census blocks and ZIP codes are not nested.
...
Second, we assigned each block to a unique ZIP code using the GIS map 
from \textcite{ESRI} based on assigning to each block the ZIP code 
...
tract-to-ZIP-code correspondence from \textcite{hudCrosswalks}.
...
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Assigning Minimum Wage Levels to USPS ZIP Codes}
...
\begin{prop}[Identification]\label{prop:PO_identification}
    ...
    \begin{equation*}
        ...
        {\partial w^{\wkp}} 
       = ACRT^{\wkp}(w^{\wkp} | w^{\res}, w^{\wkp}) 
          + \frac{\partial ATT^{\wkp}(w^{\wkp} | w^{\res}, l)}{\partial w} \Big|_{l = w^{\res}} .
    \end{equation*}
...
Consider now the functional form used in the main analysis, given in
equation \eqref{eq:func_form}. 
It is easy to see that both Assumptions \ref{assu:PT} and \ref{assu:no_selection}
hold under this linear functional form with constant effects.
...
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Estimation of the Effect of the Minimum Wage on Wage Income}
