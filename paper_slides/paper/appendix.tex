...
In every period, $\lambda_{it} L_{iz} h_{iz,t-12}$ square feet become available for 
...
\section{A Bargaining model}\label{sec:bargaining_model}

In this section, we develop a simple static model where rents are determined by 
Nash bargaining.
We will see that, if the outside option of renters depends of residence and workplace
MWs in the same way as the housing demand function in Assumption \ref{assu:housing_function},
then the same results hold.

Consider an neighborhood $i$ in which $N$ landlords with identical apartments are 
randomly matched to $N$ potential tenants.
Once the match takes place, landlords and tenants bargain over the rent.
to a new equilibrium rent $r_n^*$, while some may not reach a deal.
A new round of random matching and bargaining takes place with those who did not 
deal, and so on.
In the end, there will be a distribution of rents $\{r_n^*(\MW_{n}, \MW_{zn})\}_{n\in N}$.
We intend to show the effect of MW changes in the median of such distribution.

Consider first the Nash bargaining that happens within each round.
Worker $n$ has outside option $r^T(\MW_{i}, \MW_{zn})$, where $\MW_{i}$ is the 
residence MW and $\MW_{zn}$ the workplace one.
Set the landlords' outside option to $r^L$.
Applying standard Nash bargaing results in \textcite{AcemogluAutor2011lectures},
we can write the solution as
$$
r_{n}^* = \arg \max \left(r - r^T(\MW_{i}, \MW_{zn})\right)^\theta \left(r - r^L\right)^{1-\theta} ,
$$
or
$$
r_n^* = (1-\theta) r^T(\MW_{i}, \MW_{zn}) + \theta r^L .
$$
Note that the solution is a weighted average over the player's outside option.

Note that, while many rounds of rejections and subsequents matches are possible, 
in equilibrium there will be only one round.
Why? Consider the last period, when everyone is matched.
The previous period, a landlord will only reject an offer if 
$$r_n^{i-1} < $$
Say you are a landlord, and you have to choose between bargained rents $\tilde r$ now,
or rents next round.
