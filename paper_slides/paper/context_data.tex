...
Panel (a) focuses on state level MW policies. There are $\stateBindingMW$ states 
with a MW policy in the 2010 decade but all of them are states that already had 
a MW policy in January 2010.  
Panel (b) shows sub-state MW policies. In total, there are $\localBindingMW$ counties and 
cities with a MW policy at any moment in the 2010 decade. The number of local jurisdictions 
instituting a MW level increases strongly after 2013, and many of implement increases 
multiple times.
...
\parencite{hudSAFMR}.%
\footnote{SAFMRs data is constructed by the HUD as an extension of 
