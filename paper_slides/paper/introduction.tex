...
%% DGP: Should we mention (and advertise) the stacked design?
...
%% DGP: We used to say that commuting shares don't seems to change much based on PerezPerez, 
%% and that our results are not sensitive to those changes either. Should we consider
%% mentioning it somehwere in this version too? 
...
would not account for MW spillovers, which are central to understanding the 
distributional consequences of the rich pattern of changes in rent gradients
as a consequence of MW policy changes.
...
First, we test our identifying assumption through using leads and lags of 
each MW variable.
...
effects; and re-weighting the data to match key moments of the distribution of 
%% DGP: The reweighting and the fixed composition are not necesarilly related. 
%% I think that we should rewrite this last sentence.
...
\footnote{This result is consistent with the mechanism proposed by 
	...
	due to inneficiencies in the housing market. However, in our model we do not 
	allow for migration responses that may mitigate this coefficient in the medium run.}
...
%% DGP: This last bit seems like aother strand of literature.
