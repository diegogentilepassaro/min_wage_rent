...
increases rents by $\workplaceEst$ percent (SE=$\workplaceSe$).
A 10 percent increase in the residence MW (holding constant workplace MW) 
decreases rents by $\residenceEst$ percent (SE=$\residenceSe$). 
As a result, if both measures increase simultaneously by 10 percent then 
rents would increase by $\bothEst$ percent instead (SE=$\bothSe$).
%% SH:
