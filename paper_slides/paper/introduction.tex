...
variation even within metropolitan areas. %%% DGP: what do you think if we display the figure of percentage changes in MW's from January 2010 to Decemeber 2019 that we build for MW long run?
...
the crosswalk from UDSMapper (cite). % DGP: Note to self to fix this cites here.
...
Statistics (LODES) %DGP: Should we add a cite here as well
...
data on local wages and business income from the IRS SOI %DGP: Comment to fix cites all here
...
However, we also show results for a balanced sample from January 2010 to December 2019.
