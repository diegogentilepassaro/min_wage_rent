...
Exploring the heterogeneity of our effects we conclude two things. First, that 
we observe results they where one would expect them to be. Using data from LODES 
\parencite{CensusLODES}, we show that the effect of the residence MW measure is 
driven more by ZIP codes that are likely to have a high share of MW jobs. At the 
same time the effect of the workplace MW measure is driven more by ZIP codes with 
a high share of residents that are likely to earn a MW. 
Second, using data from \textcite{hudHousing}, we show that ZIP codes that have 
public housing availability drive results for both the residence and workplace MW 
measures. 
...
%%
%% SH: Gave it a try. I'd prefer to avoid footnotes as much as possible as
