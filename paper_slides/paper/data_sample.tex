...
First, we collect census block level data on residence and workplace area 
...
in January 2011, we compute $\underline{W_{tract}}$ defined as the total yearly wage of a full time worker 
earning the MW in a given census tract assuming that such worker works 130 hours per month during 
the 12 months of the year. \footnote{ Monthly hours needed to be considered in a full time job are obtained 
from the IRS at \url{https://www.irs.gov/affordable-care-act/employers/identifying-full-time-employees}} 
Then, for each census tract we compute in which wage bin $\underline{W_{tract}}$ falls. We estimate 
the number of MW earners in a tract as the total number of workers in all bins below that one plus 
a fraction of the total number of workers in that bin given by $\frac{\underline{W_{tract}} - \underline{b}}{\overline{b} - \underline{b}}$
where $\overline{b}$ and $\underline{b}$ represent the upper and lower limit of a given bin. Finally, we go from tract to
ZIP code as follows. From the 2010 US Census, we observe the number of houses in each census block, 
and based on our original correspondence table described in Appendix 
\ref{sec:blocks_to_uspszip}, we have assigned assign a USPS ZIP code to each census
block. \footnote{Each census block is contained in a unique census tract and that correspondence is publicly available.}
Then, we compute the number of MW earners at ZIP code $i$ as the weighted average of the number of MW 
earners in its constituent tracts, where the weights are given by the number of housing units. 
...
\footnote{We use ACS data on socio-demographic characteristics aggregated 
at the census block, and at the census tract level when income bins are included. We 
assign it to the ZIP code level as explained in \ref{sec:blocks_to_uspszip} and \ref{sec:locating_mw_workers}.
The SAFMR data are available at the ZIP code year level.}
...
observation is the county by month, ZIP code by year, and county by year.
...
level.
In the ZIP code by year and county by year panels we take, both as MW measures
and log rents variable, the yearly average of the variable in the respective 
monthly panels.
