...
program main
	...
	use "`instub'/fd_rent_panel_listrent.dta", clear

	* Static Model
	run_static_model, depvar(ln_monthlylistings_nsa_sfcc) absorb(year_month) 						///
		cluster(statefips)
...
program run_static_model
	...
	eststo clear
	eststo reg1: reghdfe D.`depvar' D.ln_mw [pw = listwgt],							///
		absorb(`absorb') 												///
	...
	eststo: reghdfe D.`depvar' D.ln_mw [pw = listwgt],									///
...
program run_dynamic_model
	...
	eststo clear
	eststo reg1: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw [pw = listwgt], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
	comment_table, trend_lin("No") trend_sq("No")
	...
	qui reghdfe D.`depvar' L(0/`w').D.ln_mw [pw = listwgt], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
	...
	eststo reg2: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw  [pw = listwgt] `if', 		///
		absorb(`absorb' i.zipcode) 							///
		vce(cluster `cluster') nocons
	comment_table, trend_lin("Yes") trend_sq("No")
	...
	eststo reg3: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw [pw = listwgt] `if',		///
...
program run_static_heterogeneity
    ...
    eststo clear
	reghdfe D.`depvar' c.d_ln_mw#i.`het_var' [pw = listwgt],							///
		absorb(`absorb') ///
		vce(cluster `cluster') nocons
...
program listrent_comp
	pwcorr medlistingpricepsqft_sfcc medrentpricepsqft_sfcc, sig
	di r(rho)^2

	

	xtreg medrentpricepsqft_sfcc medlistingpricepsqft_sfcc, fe
	reghdfe medrentpricepsqft_sfcc medlistingpricepsqft_sfcc, noabsorb nocons
	g sample = e(sample)
	foreach var in medrentpricepsqft_sfcc medlistingpricepsqft_sfcc {
		qui reghdfe `var' if sample==1, absorb(zipcode) nocons resid
		predict `var'_fe if sample==1, d 
		g `var'_c = `var' - `var'_fe if sample==1
		drop _reghdfe
	} 
	*why not higher? 
	corr medrentpricepsqft_sfcc_c medlistingpricepsqft_sfcc_c if sample==1


	reghdfe medlistingpricepsqft_sfcc monthlylistings_nsa_sfcc , nocons absorb(zipcode year_month)
	reghdfe ln_medrentprice_sfcc monthlylistings_nsa_sfcc , nocons absorb(zipcode year_month)
	reghdfe ln_medrentprice_sfcc monthlylistings_nsa_sfcc , nocons absorb(zipcode year_month)
end
