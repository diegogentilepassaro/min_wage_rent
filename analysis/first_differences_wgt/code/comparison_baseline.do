...
program main
	...
	esttab * using "`outstub'/comparison_wgt_base.tex", compress se replace 	///
		...
		mgroups("Reweighted Panel" "Baseline Panel", pattern(1 0 0 1 0 0 ) ///
...
program horse_race_models
	...
	eststo: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw [pw = wgt_cbsa100], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
		
	eststo: reghdfe D.`depvar' L(0/`w').D.ln_mw [pw = wgt_cbsa100], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
