...
program main
	...
	horse_race_models, depvar(ln_med_rent_psqft) w(3) ///
	    absorb(year_month) cluster(statefips)
	esttab * using "`outstub'/horse_race.tex", compress se replace substitute(\_ _) 	///
	    order(F3D.ln_mw F2D.ln_mw FD.ln_mw D.ln_mw LD.ln_mw L2D.ln_mw L3D.ln_mw LD.ln_med_rent_psqft) ///
        coeflabels(F3D.ln_mw "\Delta ln(MW)_{t-3}" F2D.ln_mw "\Delta ln(MW)_{t-2}" ///
		FD.ln_mw "\Delta ln(MW)_{t-1}" D.ln_mw "\Delta ln(MW)_{t}" ///
		LD.ln_mw "\Delta ln(MW)_{t+1}" L2D.ln_mw "\Delta ln(MW)_{t+2}" ///
		L3D.ln_mw "\Delta ln(MW)_{t+3}" LD.ln_med_rent_psqft "\Delta ln(y)_{t-1}") ///
