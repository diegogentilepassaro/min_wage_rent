...
program run_static_model
	...
	eststo clear
	eststo reg1: reghdfe D.`depvar' D.ln_mw [aw = wgt_cbsa100],							///
		absorb(`absorb') 												///
	...
	eststo: reghdfe D.`depvar' D.ln_mw [aw = wgt_cbsa100],									///
...
program run_dynamic_model
	...
	eststo clear
	eststo reg1: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw [aw = wgt_cbsa100], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
	comment_table, trend_lin("No") trend_sq("No")
	...
	qui reghdfe D.`depvar' L(0/`w').D.ln_mw [aw = wgt_cbsa100], 			///
		absorb(`absorb') 											///
		vce(cluster `cluster') nocons
	...
	eststo reg2: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw  [aw = wgt_cbsa100] `if', 		///
		absorb(`absorb' i.zipcode) 							///
		vce(cluster `cluster') nocons
	comment_table, trend_lin("Yes") trend_sq("No")
	...
	eststo reg3: reghdfe D.`depvar' L(-`w'/`w').D.ln_mw [aw = wgt_cbsa100] `if',		///
...
program run_static_heterogeneity
    ...
    eststo clear
	reghdfe D.`depvar' c.d_ln_mw#i.`het_var' [aw = wgt_cbsa100],							///
		absorb(`absorb') ///
		vce(cluster `cluster') nocons
