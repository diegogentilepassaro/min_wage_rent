...
The exogenous measures of each type of worker are $N_\ell$ and $N_h$.
...
pair $(n, i)$ for worker $\omega$ of type $S$ is given by
...
$$
<!-- Notes: 
- Can also include a residence amenities term in utility
...
The worker being life in metropolitan area and observes draws $b_{n i \omega}$ for 
every workplace-residence pair.
...
product of labor. When the minimum wage is set at an appropriate level, the model
can produce negative small, null, and even positive employment effects!

Note that, to simplify, we assume that firms don't use any land.
...
We make this market very simple --following much of the urban literature---. 
...
<!-- Notes:
- An obvious extension is allowing for elastic housing supply.
-->

## 2. Solving the model

### 2.1. Workers and labor supply

The distribution of indirect utility is

$$
\begin{split}
  \Pr \left( u_{ni}^s \leq u \right)
    = & \Pr \left( b_{n i} \frac{B_i w_i^s}
                  {d_{n i} Q_n^{1 - \alpha}} \leq u \right) \\
    = & \Pr \left( b_{n i} \leq \frac{d_{n i} Q_n^{1 - \alpha}}
                  {B_i w_i^s} u \right) \\
    = & e^{\Phi_{n i}^s u^{-\epsilon}} 
\end{split}
$$

where
$
\Phi_{n i}^s = -T_n^s X_i^s 
      \left(d_{n i} Q_n^{1 - \alpha} \right)^{-\epsilon} 
      \left(B_i w_i^s \right)^\epsilon .
$

Per the properties of the Fréchet distribution, the joint probability a worker 
chooses to reside in $n$ and work in $i$ is

$$
\lambda_{n i}^s 
  = \frac{\Phi_{n i}^s}{\sum_{n \in Z} \sum_{i \in Z} \Phi_{n i}^s}
  = \frac{\Phi_{n i}^s}{\Phi^s}
$$

The marginal probability of residing in $n$ or working in $i$ is simply

$$
\lambda_n^s = \sum_{i \in Z} \lambda_{n i} 
\quad \quad \quad 
\lambda_i^s = \sum_{n \in Z} \lambda_{n i}
$$

The measure of workers residing in $n$ and working in $i$ is, respectively,

$$
\tilde{L}_{n}^s = \lambda_n^s N^s
\quad \quad \quad 
\tilde{L}_{i}^s = \lambda_i^s N^s
$$
