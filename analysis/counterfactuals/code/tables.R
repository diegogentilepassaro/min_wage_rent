...
main <- function() {
  ...
  # Chicago increase to 14
  dt_cf <- dt[counterfactual == "chi14" 
              & year == 2020 
              & cbsa_low_inc_increase == 0]
  
  txt <- c("<tab:counterfactuals_chi14>")
  ...
  dt_tot_incidence <- fread(file.path(instub, "tot_incidence.csv"))
  dt_tot_incidence <- dt_tot_incidence[counterfactual == "chi14"]
  txt <- c(txt, 
           paste(dt_tot_incidence$N,
                 dt_tot_incidence$tot_incidence,
                 sep = "\t"))
  
  fileConn <- file(file.path(outstub, "counterfactuals_chi14.txt"))
  writeLines(txt, fileConn)
  close(fileConn)
