...
main <- function(){
  ...
  dt_zip_demo  <- block_demo_to_zip(instub, copy(dt_geo))
...
block_demo_to_zip <- function(instub, dt_geo) {
  
  dt <- fread(file.path(instub, "block_demo_baseline.csv"))

  dt <- dt_geo[dt,  on = c("block")]

  dt <- dt[!is.na(zipcode)]

  dt <- dt[, .(n_male_cens2010              = sum(n_male,  na.rm = T),
...
tract_to_zip <- function(instub, dt_geo) {
  ...
  dt <- fread(file.path(instub, "tract_demo_baseline.csv"))
