...
program main 
	...
	* GB: when checking 7790 notmatched from master zippanel, 95 percents are PO boxes and the remaning 5 looks like universities, 
	* 		very small rural areas. This means we probably can use zcta/zip interchangeably
	*used the below to check zipcode type 
...
program prepare_panel
	...
	keep year_month countyfips ///
		ln_mw actual_mw dactual_mw treated_mw exp_mw ln_expmw ///
		`contlist'
	...
	preserve 
		...
		save_data ../output/qcew_controls_countymonth.dta, key(countyfips year_month) replace 
	restore
