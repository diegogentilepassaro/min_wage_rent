...
main <- function(){
  ...
  log_file    <- "../output/zcta_manifest.log"
  ...
  for (yy in start_ym[1]:end_ym[1]) {
    ...
    for (mm in month_range){
      ...
      if ( (mm %in% mm_mw_changes)                              # If there was a MW change
        ...
        dt_year_zip <- rbindlist(
          list(dt_year_zip, 
               collapse_data(copy(dt), key_vars = c("zcta",   "year", "month"))))
    ...
    mw_panel_zcta <- rbindlist(list(mw_panel_zcta,  dt_year_zip))
  }
  rm(dt, dt_geo, dt_mw, dt_year_zip, dt_year_zip_prev)

  mw_panel_zcta_yearly <- mw_panel_zcta[, .(statutory_mw       = mean(statutory_mw),
                                            statutory_mw_first = statutory_mw[which(month==1)],
                                            state_mw           = mean(state_mw),
                                            state_mw_first     = state_mw[which(month==1)],
                                            county_mw          = mean(county_mw),
                                            county_mw_first    = county_mw[which(month==1)],
                                            local_mw           = mean(local_mw),
                                            local_mw_first     = local_mw[which(month==1)],
                                            binding_mw         = mean(binding_mw),
                                            binding_mw_first   = binding_mw[which(month==1)]),
                                       by = .(zcta, year)]

  ## Save data
  keep_vars <- names(mw_panel_zcta)[!grepl("sh_|cbsa", names(mw_panel_zcta))]   
  save_data(mw_panel_zcta[, ..keep_vars],   
            key      = c("zcta", "year", "month"),
            filename = file.path(outstub, "zcta_statutory_mw.dta"),
            logfile  = log_file)
  fwrite(mw_panel_zcta[, ..keep_vars],
         file = file.path(outstub, "zcta_statutory_mw.csv"))
  
  save_data(mw_panel_zcta_yearly, 
            key      = c("zcta", "year"),
            filename = file.path(outstub, "zcta_year_statutory_mw.dta"),
            logfile  = log_file)
  fwrite(mw_panel_zcta_yearly,
         file = file.path(outstub, "zcta_year_statutory_mw.csv"))
  
  ## MW Counterfactuals
  dt_cf <- compute_counterfactual(dt_blocklevel_for_cf)
  rm(dt_blocklevel_for_cf)
  
  save_data(dt_cf, key = c("zcta", "year", "month"),
            filename = file.path(outstub, "zcta_cfs.dta"),
            logfile  = log_file)
  fwrite(dt_cf, 
         file = file.path(outstub, "zcta_cfs.csv"))
}
