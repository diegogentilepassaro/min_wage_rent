...
main <- function(){
  ...
  dt.county <- fread(file.path(instub_mw, "county_statutory_mw.csv"))
  
  periods <- unique(dt.county$year_month)
  ...
  clusterExport(cl, c("dt.county", "periods", "states", "instub_mw", "instub_lodes", "outstub"), 
                    env = environment())                                # Load local environment objects in nodes

  # Build exp MW data
  dts.exp <- parLapply(cl, states, function(st) {
      
    dt.st <- assemble_expmw_state(st, periods, "actual_mw", dt.county, instub_lodes)
    return(dt.st)
  })
  ...
  dts.exp_wg_mean <- parLapply(cl, states, function(st) {
    ...
    dt.st <- assemble_expmw_state(st, periods, "actual_mw_wg_mean", dt.county, instub_lodes)
    return(dt.st)
  })
  stopCluster(cl)
  ...
  dt.exp_mw <- merge(dt.exp_mw, dt.exp_mw_wg_mean, by = c("countyfips", "year_month"))
  ...
  # Save data
  save_data(dt.exp_mw, key = c("countyfips", "year", "month"),
            filename = file.path(outstub, "county_experienced_mw.csv"),
            logfile  = log_file)
  
  save_data(dt.exp_mw, key = c("countyfips", "year", "month"),
            filename = file.path(outstub, "county_experienced_mw.dta"),
            nolog    = TRUE)
}

assemble_expmw_state <- function(x, periods, mw_var, dt.county, instub_lodes) {
   
  dt.od <- load_od_matrix(x, instub_lodes)
   
  # Computes share of treated and experienced MW for every period
  dts.period <- lapply(periods, function(y, this_st = dt.od, cty = dt.county) {
     
     dt.this_date <- cty[year_month == y, ]       # Select given date
     dt.this_date[, w_countfips := countyfips]         # Create matching variable
     
     vars_to_keep <- c("w_countyfips", mw_var, "year_month")
     dt.this_date <- dt.this_date[, ..vars_to_keep]
     
     dt.this_date <- dt.this_date[dt.od, on = 'w_countyfips'] # Paste MW to every residence(h)-workplace(w)
                                                           #  combination in 'dt.od'
     dt.this_date <- dt.this_date[!is.na(year_month),]     # Drop missings (zipcodes not showing up in mw data)
     
     dt.this_date[, h_totjob     := sum(totjob),     by = "h_countyfips"]
     dt.this_date[, h_job_young  := sum(job_young),  by = "h_countyfips"]
     dt.this_date[, h_job_lowinc := sum(job_lowinc), by = "h_countyfips"]
     ...
     dt.this_date <- dt.this_date[, 
      ...
      by = c("h_countfips", "year_month")
    ]
     
    setnames(dt.this_date, old = "h_countyfips", new = "countyfips")
...
load_od_matrix <- function(st, instub) {
   ...
   setorder(od, h_countyfips, w_countyfips)
   return(od)
}
