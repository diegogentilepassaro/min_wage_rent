...
main <- function(){
   ...
   clean_zip_place_relationship(instub  = paste0(datadir, 'zcta_place_rel_10.txt'),
                                 outstub = paste0(outputdir, 'zip_places10.csv'),
                                 key      = c('place_code', 'zcta'))
   
   clean_zip_county_relationship(instub  = paste0(datadir, 'zcta_county_rel_10.txt'),
...
clean_2010census_gazzetter <- function(instub, outstub, key) {
   ...
   varplace10 <- c('state', 'stateabb', 'place_code', 'placename', 'placetype', 'placepop10', 'landarea_sqkm')
   places10 <- places10[ ,..varplace10]
   
   save_data(df = places10, key = key, filename = outstub)
}

clean_zip_place_relationship <- function(instub, outstub, key) {
   zctaPlace10 <- fread(instub, colClasses = c("ZCTA5"="character", "PLACE"= "character", "STATE"="character"))
...
clean_zip_county_relationship <- function(instub, outstub, key) {
   zctaCounty10 <- fread(instub, colClasses = c(rep("character",4), rep("numeric", 20)))
   ...
   countyNames$countyfips<-paste0(as.character(countyNames$state_code),as.character(countyNames$county_code))
